...
class RewardScreen(private val offer: RewardOffer) : Screen(Component.literal("Daily Reward")) {
    ...
    private fun rarityToFormat(rarity: String): ChatFormatting = when(rarity) {
    ...
    private val cardBackTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/card_back.png")

    private val nodeCompletedTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/node_completed.png")
    private val nodeActiveTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/node_active.png")
    private val nodeEmptyTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/node_empty.png")
    private val nodePulseTex = (0..7).map { Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/node_active_pulse_%02d.png".format(it)) }
    private val connectorCompletedTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/connector_completed.png")
    private val connectorGradientTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/connector_gradient.png")
    private val connectorEmptyTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/connector_empty.png")
    private val connectorLiquidTex = (0..9).map { Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/connector_liquid_%02d.png".format(it)) }
    private val milestoneEmptyTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/milestone_empty.png")
    private val milestoneActiveTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/milestone_active.png")
    private val milestoneCompletedTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/milestone_completed.png")
    private val dropShadowTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/drop_shadow.png")
    private val borderPulseTex = (0..7).map { Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/streakbar/border_pulse_%02d.png".format(it)) }
    ...
    override fun extractRenderState(context: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        offer.cards.forEachIndexed { idx, card ->
            ...
            val tex: Identifier = if (flipProgress[idx] < 0.5f) cardBackTex else Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/card_${card.rarity}.png")
            ...
            context.pose().translate(centreX, drawY + drawH / 2f)
            val scaleXRaw = kotlin.math.cos(flipProgress[idx] * Math.PI).toFloat()
            val scaleX = kotlin.math.abs(scaleXRaw)
            context.pose().scale(scaleX, 1f)
            context.pose().translate(-centreX, -(drawY + drawH / 2f))
            context.blit(RenderPipelines.GUI_TEXTURED, tex, drawX, drawY, 0f, 0f, drawW, drawH, drawW, drawH)
            context.pose().popMatrix()

            if (flipProgress[idx] >= 0.5f) {
                val isLegendary = card.rarity == "legendary"
                val glowTex: Identifier = if (isLegendary) {
                    Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/glow_legendary${legendaryVariant[idx]}.png")
                } else {
                    Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/glow_${card.rarity}.png")
                }
                if (isLegendary) {
                    ...
                    context.pose().translate(-centreX, -(drawY + drawH / 2f))
                    context.blit(RenderPipelines.GUI_TEXTURED, glowTex, drawX, drawY, 0f, 0f, drawW, drawH, drawW, drawH)
                    context.pose().popMatrix()
                } else {
                    context.blit(RenderPipelines.GUI_TEXTURED, glowTex, drawX, drawY, 0f, 0f, drawW, drawH, drawW, drawH)
                }
                ...
                if (card.iconUrl.isNotEmpty()) {
                    val iconTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/icons/${card.iconUrl}.png")
                    ...
                    context.blit(RenderPipelines.GUI_TEXTURED, iconTex, iconX, iconY, 0f, 0f, iconW, iconH, iconW, iconH)
                }
                ...
                val amountY = (amountBase.toFloat() / scale).roundToInt()
                    ...
                    val adjustedAmountY = amountY
                    if (secondLine != null) {
                        adjustedNameY -= 7
                    }
                    context.centeredText(font, Component.literal(firstLine), scaledX, adjustedNameY, rarityColor)
                    if (secondLine != null) {
                        context.centeredText(font, Component.literal(secondLine), scaledX, adjustedNameY + 10, rarityColor)
                    }
                    /*
                    // downscale name to fit
                    val nameWidthPx = font.width(nameStr)
                    val maxNameWidth = cardWidth - 10
                    val nameScale = if (nameWidthPx > maxNameWidth) maxNameWidth.toFloat() / nameWidthPx else 1f
                    context.pose().pushMatrix()
                    context.pose().scale(nameScale, nameScale, 1f)
                    val scaledXName = (scaledX / nameScale).roundToInt()
                    val scaledYName = (nameY / nameScale).roundToInt()
                    context.centeredText(font, Component.literal(nameStr), scaledXName, scaledYName, rarityColor)
                    context.pose().popMatrix() */
                    context.centeredText(font, Component.literal(amountStr), scaledX, adjustedAmountY, rarityColor)
                    context.pose().popMatrix()
                        ...
                        val rarityFormat = rarityToFormat(card.rarity)
                            val tooltip = listOf(
                                Component.literal("Rarity: ")
                                    .append(Component.literal(card.rarity.uppercase())
                                        .withStyle(rarityFormat, ChatFormatting.BOLD)),
                                Component.literal(card.description)
                            )
        ...
        context.centeredText(font, Component.literal(streakText), width / 2, y + cardHeight + 15, 0xFFFFFFFF.toInt())

        renderStreakBar(context, y + cardHeight + 35)

        context.centeredText(
            font,
            Component.literal(claimLabel).withStyle(ChatFormatting.GOLD, ChatFormatting.BOLD),
            width / 2,
            y + cardHeight - 257,
            0xFFFFFFFF.toInt()
        )
    ...
    private fun renderStreakBar(context: GuiGraphicsExtractor, baseY: Int) {
        val nodeSize = 22
        val spacing = 12
        val thickness = 6
        val nodes = 9
        ...
        for (i in 0 until 8) {
            ...
            context.centeredText(font, Component.literal(idx.toString()), centers[i], centerY - 4, textColor)
        }
        ...
        if (showMilestoneGlow && !isMilestoneClaimAnim) {
            val glowTex = Identifier.fromNamespaceAndPath("dailyrewards", "textures/gui/glow_legendary2.png")
            val t = (now % 2000L).toFloat() / 2000f
            ...
            context.pose().pushMatrix()
            context.pose().translate(mCenterX.toFloat(), centerY.toFloat())
            context.pose().scale(scaleVariation, scaleVariation)
            context.pose().rotate(Math.toRadians(rotationAngle.toDouble()).toFloat())
            context.pose().translate(-mCenterX.toFloat(), -centerY.toFloat())
            context.blit(
        ...
        offer.cards.forEachIndexed { idx, _ ->
            ...
            if (mouseX >= x && mouseX <= x + cardWidth && mouseY >= y && mouseY <= y + cardHeight) {
                if (!revealed[idx]) {
                    ...
                    Minecraft.getInstance().soundManager.play(SimpleSoundInstance.forLocalAmbience(ModSoundEvents.PICK, 1f, 1f))
                    if (offer.id != "debug") {
                    ...
                    val rarityFormat = rarityToFormat(card.rarity)
                    val msg: MutableComponent = MessageUtils.PREFIX()
                        .withStyle(ChatFormatting.WHITE)
                        .append(Component.literal("Claiming reward #${idx + 1}: ").withStyle(ChatFormatting.RESET))
                        .append(Component.literal("${card.rarity.uppercase()} ").withStyle(rarityFormat, ChatFormatting.BOLD))
                        .append(Component.literal(card.name).withStyle(rarityFormat))
                        .append(Component.literal(" x${card.amount}").withStyle(ChatFormatting.RESET))
    ...
    private fun renderMilestoneClaimAnimation(context: GuiGraphicsExtractor, centerY: Int, radius: Int) {
        ...
        if (textProgress > 0f) {
            val text = Component.literal("§6+1 Daily Reward Token")
            ...
            context.text(font, text, textX, textY.toInt(), textColor, true)
        }
