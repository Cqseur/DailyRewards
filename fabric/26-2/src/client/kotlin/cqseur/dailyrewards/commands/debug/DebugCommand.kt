...
object DebugCommand {
    ...
    private fun registerDebugCommands(dispatcher: CommandDispatcher<FabricClientCommandSource>) {
        dispatcher.register(
            ClientCommands.literal("dailyrewards-debug")
                .requires { hasDeveloperAccess() }
                .then(ClientCommands.literal("timezone")
                    .executes { context ->
                        showTimezoneDebug()
                        1
                    }
                )
                .then(ClientCommands.literal("times")
                    .executes { context ->
                        showDisplayTimes()
                        1
                    }
        ...
        dispatcher.register(
            ClientCommands.literal("debugcards")
                .requires { hasDeveloperAccess() }
                .executes {
                    generateDebugCards()
                }
                .then(ClientCommands.literal("streak")
